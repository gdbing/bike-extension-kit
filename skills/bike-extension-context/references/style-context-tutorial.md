...
Each editor style is an ordered list of rules, organized into layer groups. A rule is composed of a relative [outline path](https://bikeguide.hogbaysoftware.com/bike-2-preview/using-bike/using-outline-paths) and a callback function. The callback function is passed the editor state and a style object to modify. The purpose of layer groups is to allow rules to be inserted into (or included from) existing editor styles.
...
1. The matching outline path is calling the `selection` function with a `block` parameter value. This function will return true if the current row has block selection. This and other outline path functions are documented in [outline paths](https://bikeguide.hogbaysoftware.com/bike-2-preview/using-bike/using-outline-paths).
